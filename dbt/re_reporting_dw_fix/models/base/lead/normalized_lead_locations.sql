...
SELECT
	 ...
	 {{ parse_json('normalized_purchase_location') }} as normalized_purchase_location,
	 {{ parse_json('normalized_sell_location') }} as normalized_sell_location,
	 {{ local_convert_timezone('created','CETDST') }} as created,
	 {{ local_convert_timezone('updated','CETDST') }} as updated,
	 purchase_state,
	 sell_state
FROM {{source('public', 'raw_import_normalized_lead_locations')}}
{% if is_incremental() %}
