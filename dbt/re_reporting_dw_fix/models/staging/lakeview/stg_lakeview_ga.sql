...
added_cols AS (
  ...
  FROM {{ source('public', 'lakeview_prod_ga_data') }}
)
, dedup AS (
	SELECT
		ga_date,
	...
	FROM added_cols
	GROUP BY ga_date, ga_pagepath, ga_sourceMedium, ga_campaign
...
,agg AS(
	...
	FROM dedup
	GROUP BY ga_date, ga_sourceMedium, ga_campaign
	ORDER BY ga_date desc
)
,users AS(
	SELECT
		...
		ga_date,
		 ga_sourceMedium, 
		 ga_campaign
	FROM {{ source('public', 'lakeview_users') }}
	GROUP BY ga_date, ga_sourceMedium, ga_campaign
	ORDER BY ga_date desc
)
SELECT 
...
FROM agg
INNER JOIN users
ON users.ga_date = agg.ga_date and users.ga_sourceMedium = agg.ga_sourceMedium and users.ga_campaign = agg.ga_campaign
ORDER BY ga_date DESC