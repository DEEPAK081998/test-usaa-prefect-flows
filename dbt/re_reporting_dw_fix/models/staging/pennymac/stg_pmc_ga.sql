...
{#- create two columns for bounces and total_time on sec for fitering  -#}
WITH 
...
added_cols AS (
  ...
  FROM {{ source('public', 'pennymac_rep60_prod_ga_data') }} 
)
{#-
DEDUPLICATING earlier collected data
filtering by pagepath is possible here SQL level with a WHERE statement or 
refactoring and removing join statement at the end (user counts will be off)
-#}
, dedup AS (
	SELECT
		ga_date,
...
,users AS(
	SELECT
		...
		ga_date
	FROM {{ source('public', 'pennymac_rep60_users') }} 
	GROUP BY ga_date
	ORDER BY ga_date desc
)
SELECT 
