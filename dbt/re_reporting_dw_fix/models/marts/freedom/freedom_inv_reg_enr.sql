...
FROM {{ ref('stg_freedom_registrations') }} sfr
FULL OUTER JOIN {{ ref('stg_freedom_invites') }} sfi
ON sfr.customer_email = sfi.email
),
freedom_enrollments AS(
	SELECT 
...
combine AS(
	...
	FULL OUTER JOIN freedom_enrollments
	ON lower(freedom_enrollments.client_email) = lower(registration_inv.all_email)
),
final AS(
	SELECT 
