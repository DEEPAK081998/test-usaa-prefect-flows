...
final_cte AS (
    ...
    WHERE data::json->>'newContent' like '%OON%' and role = 'ADMIN'

    UNION ALL

    SELECT DISTINCT lead_id,0, 1,0
    FROM {{ ref('note_updates') }} 
    WHERE data::json->>'newContent' like '%Fail: C%' and role = 'ADMIN'

    UNION ALL

    SELECT DISTINCT lead_id,0, 0,1
    FROM {{ ref('note_updates') }} 
    WHERE data::json->>'newContent' like '%Fail: D%' and role = 'ADMIN'
)
SELECT 
  lead_id,
