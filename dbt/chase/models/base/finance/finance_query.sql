WITH leads_cte AS(
	SELECT
		DISTINCT(ld.lead_id),
        (json_array_elements(ld.data->'closingProcess'->'closingDetails')->>'createdAt')::bigint as update_date,
        ...
        cast(nullif(replace(json_array_elements(ld.data->'closingProcess'->'closingDetails')->>'salePrice',',',''),'')as numeric) as SalePrice,
        ...
        cast(nullif(replace(json_array_elements(ld.data->'closingProcess'->'closingDetails')->>'agentCommissionAmount',',',''),'') as decimal) as AgentCommissionAmount,
        cast(nullif(nullif(replace(json_array_elements(ld.data->'closingProcess'->'closingDetails')->>'agentCommissionPercentage',',',''),''),'N/A') as decimal) as Commissionpercent,
        ...
        json_array_elements(ld.data->'closingProcess'->'closingDetails')->>'isBuyer' as BuyTransaction
	FROM
		{{ ref('leads_data') }} ld
...
select 
	...
	ff.row_id
from final_f ff
left join {{ ref('leads') }} l on l.id = ff.lead_id 
LEFT OUTER JOIN (
    SELECT
		lsu.lead_id,
