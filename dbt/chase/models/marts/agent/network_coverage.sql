...
agent_count_cte AS (
    ...
    join {{ ref('cbsa_locations') }} cb on lpad(cb.zip,5,'0') = puz.zip
...
,leads_by_profile  as (
select lead_id,profile_aggregate_id from current_assignments
where ROLE ilike '%brok%'
group by 1,2
)
, leads_by_brokerage AS (

select distinct lbp.lead_id ,b.brokerage_code from brokerages b 
...
, lead_count_cte AS (
   ...
   --     cte.brokerage_code,
    ...
    join {{ ref('cbsa_locations') }} cb 
        on 
        lpad(cb.zip,5,'0') = COALESCE(nl.normalized_sell_location->>'zip',nl.normalized_purchase_location->>'zip')
...
, brokarage_coverage_zip_cte AS (
    select 
        ...
        cb.st,
        bcz.brokerage_code,
        COALESCE(b.full_name,pup.data->'brokerage'->>'fullName') as brokerageName,
        count(distinct bcz.brokerage_code) as brokeragecount
    from {{ ref('brokerage_coverage_zips') }} bcz
    join {{ ref('cbsa_locations') }} cb on lpad(cb.zip,5,'0') = bcz.zip
    left join {{ ref('brokerages') }} b on b.brokerage_code = bcz.brokerage_code
    left join {{ ref('partner_user_profiles') }} pup on pup.brokerage_code = b.brokerage_code
    group by cb.county,cb.st,bcz.brokerage_code, brokerageName
)
select 
cbsa.st,
cbsa.county,
