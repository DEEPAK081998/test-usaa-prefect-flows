...
with activity_agent_bank_cte as (
    ...
    select 
        ...
        count(distinct case when lower(agentactivity) ='accepted' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_acceptcount,
        count(distinct case when lower(agentactivity) ='reject' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_rejectcount,
        count(distinct case when lower(agentactivity) ='offered' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_offercount,
        count(distinct case when lower(agentactivity) ='ctc' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_ctccount,
        count(distinct case when lower(agentactivity) ='timeout' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_timeoutcount,
        count(distinct case when lower(agentactivity) ='update' and {{ date_diff_columns('day','aa.enrolleddate') }}<=30 then lead_id end) last30days_agentbank_statuscount,
        count(distinct case when lower(agentactivity) ='accepted' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_acceptcount,
        count(distinct case when lower(agentactivity) ='reject' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_rejectcount,
        count(distinct case when lower(agentactivity) ='offered' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_offercount,
        count(distinct case when lower(agentactivity) ='ctc' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_ctccount,
        count(distinct case when lower(agentactivity) ='timeout' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_timeoutcount,
        count(distinct case when lower(agentactivity) ='update' and {{ date_diff_columns('day','aa.enrolleddate') }}<=90 then lead_id end) last90days_agentbank_statuscount,
        count(distinct case when lower(agentactivity) = 'accepted' and {{ date_diff_columns('day','enrolleddate') }} between 90 and 456 then lead_id end) as l15mo_agentbank_acceptcount,
        count(distinct case when lower(agentactivity) = 'accepted' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agentbank_acceptcount,
        count(distinct case when lower(agentactivity) = 'reject' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agentbank_rejectcount,
        count(distinct case when lower(agentactivity) = 'offered' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agentbank_offeredcount
...
activity_agent_cte as (
    select 
        ...
        count(distinct case when lower(agentactivity) = 'accepted' and {{ date_diff_columns('day','enrolleddate') }} between 90 and 456 then lead_id end) as l15mo_agent_acceptcount,
        count(distinct case when lower(agentactivity) = 'accepted' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agent_acceptcount,
        count(distinct case when lower(agentactivity) = 'reject' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agent_rejectcount,
        count(distinct case when lower(agentactivity) = 'offered' and {{ date_diff_columns('day','enrolleddate') }} <=365 then lead_id end) as l12mo_agent_offeredcount
    from 
        {{ ref('fct_agent_referral_activities') }} fr 
        join {{ ref('partner_user_profiles') }} pup on fr.agent_aggregate_id = pup.aggregate_id
    group by 
        fr.agent_aggregate_id
),
activity_cte as (
...
close_referrals_agent_cte as (
    select 
        agentAggregateId,
        count(distinct case when lower(majorstatus) = 'closed' and {{ date_diff_columns('day','enrolleddate') }} between 90 and 456 then id end) as l15mo_agent_closecount
    from 
        {{ ref("chase_enrollments_test") }} ld
    group by 
        agentAggregateId
),
close_referrals_agentbank_cte as (
    select 
        agentAggregateId,
        bank_id,
        count(distinct case when lower(majorstatus) = 'closed' and {{ date_diff_columns('day','enrolleddate') }} between 90 and 456 then id end) as l15mo_agentbank_closecount
    from 
        {{ ref("chase_enrollments_test") }} ld
    group by 
        agentAggregateId,
...
referral_agent_bank_cte as (
    ...
    select 
        ...
        count(distinct case when {{ date_diff_columns('day','t1.enrolleddate') }} >= 90 then t1.id end) as agentbank_currentreferrals_over90days,
        count(distinct case when {{ date_diff_columns('day','t1.enrolleddate') }} <= 30 then t1.id end) as agentbank_currentreferrals_acceptedlast30days,
        count(distinct case when {{ date_diff_columns('day','t1.enrolleddate') }} <= 90 then t1.id end) as agentbank_currentreferrals_acceptedlast90days,
        count(distinct case when lower(majorstatus) = 'active' then t1.id end) as agentbank_activereferrals,
        count(distinct case when lower(majorstatus) = 'closed' then t1.id end) as agentbank_closedreferrals,
        count(distinct case when lower(majorstatus) ='closed' and {{ date_diff_columns('day','t1.enrolleddate') }} >= 90 then t1.id end) as agentbank_closedreferrals_over90days,
        count(distinct case when lower(majorstatus) = 'inactive' then t1.id end) as agentbank_inactivereferrals,
        count(distinct case when lower(majorstatus) = 'pending' then t1.id end) as agentbank_pendingreferrals,
...
referrals_cte as (
	...
	from
        ...
        left join close_referrals_agent_cte cl_ag_ref 
            on ab_ref.agent_aggregate_id = cl_ag_ref.agentAggregateId
        left join close_referrals_agentbank_cte cl_agbk_ref 
            on ab_ref.agent_aggregate_id = cl_agbk_ref.agentAggregateId 
